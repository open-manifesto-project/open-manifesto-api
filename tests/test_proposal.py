...
class ProposalTests(unittest.TestCase, FixturesMixin):
    ...
    def test_list_proposals_filters_manifesto(self):
        response = self.client.get('/proposal?political_party=Compromiso por Europa')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.get_json()), 166)
        response = self.client.get('/proposal?political_party=Otro')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.get_json()), 0)

    def test_list_proposals_filters_agents(self):
        response = self.client.get('/proposal?agents=test')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.get_json()), 2)
        self.assertEqual(sorted(response.get_json()[0].keys()), self.proposal_keys)

    def test_list_proposals_filters_tags(self):
        response = self.client.get('/proposal?tags=energía,educación')
        self.assertEqual(response.status_code, 200)
    ...
    def test_list_proposals_filters_tags_threshold(self):
        # threshold = 0
        response = self.client.get('/proposal?tags=europea,social,cultura')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.get_json()), 64)
        # threshold = 0.5
        response = self.client.get('/proposal?tags=europea,social,cultura&threshold=0.5')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.get_json()), 11)
        # threshold = 0.5
        response = self.client.get('/proposal?tags=europea,social,cultura&threshold=1')
        self.assertEqual(response.status_code, 200)
    ...
    def test_get_topics(self):
        response = self.client.get('/proposal/topic')
        self.assertEqual(response.status_code, 200)
        self.assertTrue(isinstance(response.get_json(), list))

    def test_get_priorities(self):
        response = self.client.get('/proposal/priority')
        self.assertEqual(response.status_code, 200)
        self.assertTrue(isinstance(response.get_json(), list))
